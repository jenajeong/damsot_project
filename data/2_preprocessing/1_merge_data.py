...
# # 숫자형 변환
df2['결제 합계'] = int_format(df2, '결제 합계')
df1['매출'] = int_format(df1, '매출')
...
df = pd.merge(df1, df2, on='판매일시', how='left')

fin_df = df[['판매일시','테이블코드','타입','결제 합계','상품명','상품코드','수량','매출','공급가액','부가세']]
fin_df = fin_df.drop_duplicates(keep='first').reset_index(drop=True)
